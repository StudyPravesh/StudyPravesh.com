...
class Mage_Eav_Model_Resource_Entity_Attribute extends Mage_Core_Model_Resource_Db_Abstract
{
    ...
    protected function _saveOption(Mage_Core_Model_Abstract $object)
    {
        ...
        if (is_array($option)) {
			...
			$parent_attribute_id 	= !empty($option['parent_attribute_id'][$optionId]) ? $option['parent_attribute_id'][$optionId] : 0;		//	S:VA
                    if (!$intOptionId) {
                        $data = array(
                           'attribute_id'  		=> $object->getId(),
                           'sort_order'    		=> $sortOrder,
						   'parent_attribute_id'=> $parent_attribute_id		//	S:VA
                        );
                        $adapter->insert($optionTable, $data);
                        $intOptionId = $adapter->lastInsertId($optionTable);
                    } else {
						$data  = array('sort_order' => $sortOrder, 'parent_attribute_id' => $parent_attribute_id);	//	S:VA
